...
func createGet() []byte {
	...
	ret = append(ret, id...)
	lastDelta := 0
	option, lastDelta := createOption("uri", []byte("test"), lastDelta)
...
func createPost() []byte {
	...
	ret = append(ret, id...)
	lastDelta := 0
	option, lastDelta := createOption("uri", []byte("sink"), lastDelta)
	ret = append(ret, option...)
	option, _ = createOption("contentType", make([]byte, 0), lastDelta)
	ret = append(ret, option...)
	ret = append(ret, byte(0xFF))
	ret = append(ret, []byte("23")...)
